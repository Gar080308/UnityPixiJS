...
public class ExportSceneToJSON : Editor
{
    ...
    public static async void Export()
    {
        ...
        foreach (var obj in _objectDict)
        {
            ...
            var graph = obj.Value.GetComponent<LineRenderer>();
                ...
                else
                {
                    if (!obj.Value.GetComponent<Animator>())
                    {
                        if(img.type != Image.Type.Filled)
                            ...
                            s += $"const {obj.Key} = new TilingSprite{threeD}(sheet.textures[\"{img.sprite.name}.png\"], {obj.Value.sizeDelta.x},{obj.Value.sizeDelta.y});";
                    }
                    else
                    {
            ...
            if (text != null)
            {
                ...
                {
                    s += $"const {obj.Key} = new Text{threeD}('{text.text}', new TextStyle({{ fill : ['#{ColorUtility.ToHtmlStringRGB( text.color )}'], wordWrap: true, wordWrapWidth: {text.rectTransform.sizeDelta.x},lineHeight : {(int)(text.fontSize * 1.5f)} ,fontFamily: 'Baloo', fontSize: {text.fontSize} }}));";
                }
